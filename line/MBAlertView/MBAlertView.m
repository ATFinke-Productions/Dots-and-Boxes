...
+(MBAlertView*)alertWithBody:(NSString*)body cancelTitle:(NSString*)cancelTitle cancelBlock:(void (^)())cancelBlock
{
    ...
    alert.bodyText = body;
    if(cancelTitle)
        [alert addButtonWithText:cancelTitle type:MBAlertViewItemTypePositive block:cancelBlock];
    return alert;
}
+(MBAlertView*)errorWithBody:(NSString *)body cancelTitle:(NSString *)cancelTitle cancelBlock:(void (^)())cancelBlock{
    MBAlertView *alert = [[MBAlertView alloc] init];
    alert.bodyText = body;
    if(cancelTitle)
...
-(void)centerViews
{
    [_buttons enumerateObjectsUsingBlock:^(MBAlertViewButton *button, NSUInteger idx, BOOL *stop)
    {
        if(_imageView)
        {
            [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : _imageView, @"offset" : @0.0f}, @{@"view" : self.bodyLabelButton, @"offset" : @20.0f}, @{@"view" : button, @"offset" : @20.0f}]];
        }
        else
        {
            [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : self.bodyLabelButton, @"offset" : @0.0f}, @{@"view" : button, @"offset" : @20.0f}]];
        }
    }];
}
...
-(void)performLayoutOfButtons
{
    ...
    [self.items enumerateObjectsUsingBlock:^(MBAlertViewItem *item, NSUInteger index, BOOL *stop)
     {
         MBAlertViewButton *buttonLabel = _buttons[index];
         float origin = 0;
         if(index == 0)
