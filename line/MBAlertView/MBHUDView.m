...
-(void)centerViews
{
    if(_imageView)
    {
        [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : self.imageView, @"offset" : @(self.iconOffset.height)}, @{@"view" : self.bodyLabelButton, @"offset" : @(10 + _bodyOffset.height)}]];
    }
    else if(_hudType == MBAlertViewHUDTypeActivityIndicator)
    {
        [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : _activityIndicator, @"offset" : @(self.iconOffset.height)}, @{@"view" : self.bodyLabelButton, @"offset" : @0.0f}]];
    }
    else if(_hudType == MBAlertViewHUDTypeCheckmark)
    {
        [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : _checkMark, @"offset" : @(self.iconOffset.height)}, @{@"view" : self.bodyLabelButton, @"offset" : @10.0f}]];
    }
    else if(_hudType == MBAlertViewHUDTypeLabelIcon || self.iconLabel.text)
    {
        [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : self.iconLabel, @"offset" : @(self.iconOffset.height)}, @{@"view" : self.bodyLabelButton, @"offset" : @(self.bodyOffset.height)}]];
    }
    else
    {
        [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : _bodyLabelButton, @"offset" : @(self.bodyOffset.height)}]];
    }
