...
-(void) submitScore:(int64_t)score {
    ...
    GKScore* gkScore = [[GKScore alloc] initWithCategory:@"com.atfp.dot.win"];
...
-(void)authenticateLocalUserFromViewController:(UIViewController *)authenticationPresentingViewController {
    ...
    __block GKLocalPlayer * localDude = localPlayer;
    ...
    localPlayer.authenticateHandler = ^(UIViewController *viewController, NSError *error)
    {
        ...
        } else if (localDude.authenticated) {
            userAuthenticated = YES;
            //2
            GKTurnBasedEventHandler *ev = [GKTurnBasedEventHandler sharedTurnBasedEventHandler];
            ev.delegate = self;
...
-(void)handleTurnEventForMatch:(GKTurnBasedMatch *)match didBecomeActive:(BOOL)didBecomeActive {
    ...
    if ([match.matchID isEqualToString:self.currentMatch.matchID]) {
        ...
        if ([match.currentParticipant.playerID isEqualToString:[GKLocalPlayer localPlayer].playerID]) {
            ...
            [self.delegate sendNotice:@"It's your turn in another match." forMatch:match];
        } else {
...
-(void)handleMatchEnded:(GKTurnBasedMatch *)match {
    if ([match.matchID isEqualToString:self.currentMatch.matchID]) {
        ...
        [self.delegate sendNotice:@"One of your current matches just finished" forMatch:match];
    }
